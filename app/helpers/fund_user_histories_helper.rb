module FundUserHistoriesHelper
  def buy_date_for_new
    @fund_user_history.buy_date ? @fund_user_history.buy_date : Time.current.to_date
  end
  ...
  def card_id_for_fund_user_history
    if @fund_user_history.card
      ...
      if @fund_user.fund_user_histories.where.not(card_id: nil).exists?
        @fund_user.fund_user_histories.where.not(card_id: nil).first.card_id
      end
    end
  end
