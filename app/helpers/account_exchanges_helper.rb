module AccountExchangesHelper
  def to_id_for_new_ax
    if @ax.to_account
      ...
      if current_user.account_exchanges.where.not(to_account: nil).exists?
        current_user.account_exchanges.where.not(to_account: nil).first.to_id
      end
    end
  end
  ...
  def card_id_for_ax
    if @ax.card
      ...
      if current_user.account_exchanges.where.not(card_id: nil).exists?
        current_user.account_exchanges.where.not(card_id: nil).first.card_id
      end
    end
  end
  ...
  def account_id_for_ax
    if @ax.account
      ...
      if current_user.account_exchanges.where.not(source_id: nil).exists?
        current_user.account_exchanges.where.not(source_id: nil).first.source_id
      end
    end
  end
