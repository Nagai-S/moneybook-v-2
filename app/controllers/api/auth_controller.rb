class Api::AuthController < Api::ApplicationController
  ...
  def initial_regist_db
    if params["kind"] == "0"
      event = User.first.events.create(
        date: Date.new(
          params["date(1i)"].to_i,
          params["date(2i)"].to_i,
          params["date(3i)"].to_i
      ...
      ax = User.first.account_exchanges.create(
        date: Date.new(
          params["date(1i)"].to_i,
          params["date(2i)"].to_i,
          params["date(3i)"].to_i
      ...
      fund_user = User.first.fund_users.create(
        fund_id: params["fund_id"],
        average_buy_value: params["average_buy_value"]
      )
      params["fuh"].each do |fuh|
        new_fuh = fund_user.fund_user_histories.create(
          date: Date.new(
            fuh["date(1i)"].to_i,
            fuh["date(2i)"].to_i,
            fuh["date(3i)"].to_i
