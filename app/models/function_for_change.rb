  ...
  def after_change_action(selected_pay_date)
    ...
    if self.account
      ...
      if selected_pay_date
        pay_day=FlexDate.return_date(selected_pay_date.year,selected_pay_date.month,self.card.pay_date)
      else
  ...
  def decide_pay_day
    if self.card.pay_date > self.card.month_date
      if self.card.month_date < self.date.day
        ...
        pay_day=FlexDate.return_date(a.year, a.month, self.card.pay_date)
      else
        pay_day=FlexDate.return_date(event_date.year, event_date.month, self.card.pay_date)
      end
    else
      if self.card.month_date < self.date.day
