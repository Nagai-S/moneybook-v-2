...
RSpec.describe FundUserHistoriesController do
  ...
  describe "create" do
    ...
    describe "association_model_update" do
      ...
      it "正しいcardで登録される" do
        ...
        expect(FundUserHistory.order(:id).last.account_id).to eq @card1.account_id
  ...
  describe "destroy" do
    it "正しいuserでなくて失敗" do
      ...
      expect{
        delete :destroy, params: {fund_user_id: @fund_user.id, id: fund_user_history.id}
      }.to change{
        FundUserHistory.all.length
      }.by(0)
    end
     ...
     describe "updateに失敗" do
      it "valueがなくて失敗するがaccount, cardに変化がない" do
        ...
        expect{
          ...
          FundUserHistory.find(fund_user_history.id).card_id
        }.by(0)
      end
