...
RSpec.describe CardsController do
  before do
    ...
    fund = Fund.create(name: 'aaa', value: 10_000)
    @fund_user =
      @user.fund_users.create(fund_id: fund.id, average_buy_value: 12_000)
  end
  ...
  describe 'update' do
    it 'userが正しくなくて失敗' do
      ...
      }.by(0).and change { Card.find(@card1.id).month_date }.by(0)
    end
  end
  ...
  describe 'destroy' do
    ...
    it '未引き落としイベントがあってdestroyできない' do
      ...
      expect { delete :destroy, params: params }.to change { Card.all.length }
        .by(0)
    end
