      ...
      context "購入" do
        it "accountとcardがともにnilでcreate" do
          ...
          expect{post :create, params: params}.to change{
            ...
            Account.find(@account.id).value
          }.by(0).and change{
            Card.find(@card.id).account.value
          }.by(0)
        end
    ...
    context "成功" do
      context "購入" do
        ...
        it "引き落とし済みの時cardに紐づく対象アカウントに購入分が戻ってくる" do
          ...
          fund_user_history.change_pon(fund_user_history.pon)
          params[:id] = fund_user_history.id
          expect{delete :destroy, params: params}.to change{
            FundUserHistory.all.length
          }.by(-1).and change{
        ...
        it "未引き落としの時account.valueに変化はなくaccount.after_pay_valueが購入分増える" do
          fund_user_history = @fund_user.fund_user_histories.create(
            buy_or_sell: true,
            commission: 100,
            date: Date.today,
