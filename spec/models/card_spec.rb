...
RSpec.describe Card do
  before do
    @user = create(:user)
    @user.confirm
    @account1 = @user.accounts.create(name: "account1", value: 10000)
    @account2 = @user.accounts.create(name: "account2", value: 10000)
    ...
    @card_for_destroy = @user.cards.create(
    ...
    @ax3 = @user.account_exchanges.create(
      date: Date.today.prev_month(2),
      source_id: nil,
      card_id: @card_for_destroy.id,
      to_id: @account1.id,
    ...
    @fund = Fund.create(name: "fund", value: 10000)
    @fund_user = @user.fund_users.create(
      fund_id: @fund.id,
      average_buy_value: 9000,
    )
    @fund_user_history1 = @fund_user.fund_user_histories.create(
      date: Date.today,
      buy_or_sell: true,
      commission: 150,
      value: 1000,
    ...
    @fund_user_history2 = @fund_user.fund_user_histories.create(
    ...
    @fund_user_history3 = @fund_user.fund_user_histories.create(
      date: Date.today.prev_month(3),
      buy_or_sell: true,
      commission: 250,
      value: 2000,
  ...
  describe "for pay_not_for_card" do
    it "#not_pay_dates" do
      expect(@card.not_pay_dates).to eq [
        ...
        Date.new(Date.today.year, Date.today.month + 1, 27),
      ]
    end
    ...
    it "not_pay_value" do
      ...
      expect(
        @card.not_pay_value(Date.new(Date.today.year, Date.today.month + 1, 27))
      ).to eq 1100
    end
  end
  ...
  describe "with_controller" do
    ...
    it "#before_destroy_action" do
      ...
      expect(Event.find(@event3.id).account_id).to eq @card_for_destroy.account_id
      expect(AccountExchange.find(@ax3.id).card_id).to eq nil
      expect(AccountExchange.find(@ax3.id).source_id).to eq @card_for_destroy.account_id
      expect(FundUserHistory.find(@fund_user_history3.id).card_id).to eq nil
      expect(FundUserHistory.find(@fund_user_history3.id).account_id).to eq @card_for_destroy.account_id
    end
  end
